...
var _ObjCClass = require('cocoascript-class-babel-safe')
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (!DelegateClass) {
      DelegateClass = ObjCClass({
        ...
        'connection:canAuthenticateAgainstProtectionSpace:': function (connection, protectionSpace) {
          return protectionSpace.authenticationMethod().isEqualToString(NSURLAuthenticationMethodServerTrust)
        },
        'connection:didReceiveAuthenticationChallenge:': function (connection, challenge) {
          var isServerTrustChallenge = challenge.protectionSpace().authenticationMethod().isEqualToString(NSURLAuthenticationMethodServerTrust)
          if (isServerTrustChallenge && this.trustedHosts.containsObject(challenge.protectionSpace().host())) {
            challenge.sender().useCredential_forAuthenticationChallenge(
              NSURLCredential.credentialForTrust(challenge.protectionSpace().serverTrust()), challenge
            )
          } else {
    ...
    connectionDelegate.trustedHosts = NSArray.alloc().initWithArray(options.trustedHosts || [])
